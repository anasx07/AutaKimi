...
export default function ExtensionsManager(): React.JSX.Element {
  const { showNsfw, selectedLangs, setSelectedLangs } = useSettingsStore()
  ...
  const [bulkInstallStatus, setBulkInstallStatus] = useState<{
  ...
  const uniqueLangs = [
    ...new Set(extensions.map((ext) => ext.lang).filter((l) => l && l.toLowerCase() !== 'all'))
  ].sort()
  ...
  const filteredExtensions = extensions
    .filter((ext) => {
      ...
      const matchesLang =
        selectedLangs.includes('all') ||
        selectedLangs.length === 0 ||
        selectedLangs.includes(ext.lang)
  ...
  const executeBulkInstall = async (toInstall: Extension[]) => {
    ...
    await Promise.all([loadFromDb(), loadInstalled()])
      ...
      <div className="shrink-0 flex flex-col space-y-6">
        <div className="flex flex-col space-y-2">
          ...
          <p className="text-muted-foreground">
            Discover and install new sources to expand your Manga library.
          </p>
        </div>
            ...
            <Button
              ...
              onClick={() => {
                if (extensionSortBy === 'supported')
                  ...
                  setExtensionSortBy('supported')
                  setExtensionSortOrder('asc')
                }
              }}
              className={cn(
                'h-8 gap-2 text-xs',
            ...
            <Button
              ...
              onClick={() => {
                if (extensionSortBy === 'installed')
                  ...
                  setExtensionSortBy('installed')
                  setExtensionSortOrder('asc')
                }
              }}
              className={cn(
                'h-8 gap-2 text-xs',
                extensionSortBy === 'installed'
                  ? 'bg-primary/10 text-primary hover:bg-primary/20'
                  : 'text-muted-foreground'
              )}
            >
        ...
        {bulkInstallStatus && (
          <div className="max-w-2xl mx-auto space-y-2 animate-in fade-in slide-in-from-top-4">
            ...
            <p className="text-[10px] uppercase tracking-widest font-black text-center text-muted-foreground">
              Bulk Installation Progress:{' '}
              {Math.round((bulkInstallStatus.current / bulkInstallStatus.total) * 100)}%
            </p>
          </div>
        )}
      </div>
        ...
        <div className="space-y-6 pb-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredExtensions.map((ext) => (
              <Card
                key={ext.pkg}
                className="group p-4 hover:bg-secondary/40 transition-all duration-200"
              >
                  ...
                  <div className="flex-1 space-y-1">
                    <div className="flex items-center justify-between">
                      <div className="flex flex-col">
                        ...
                        <p className="text-[10px] text-muted-foreground truncate opacity-70 max-w-[120px]">
                          {ext.pkg}
                        </p>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <Badge
                          variant={ext.nsfw ? 'destructive' : 'secondary'}
                    ...
                    <div className="flex flex-col gap-1.5 overflow-hidden">
                      {(() => {
                        ...
                        if (native) {
                          const isHandwritten =
                            !(generatedSourcesJson as any)[ext.pkg] ||
                            (generatedSourcesJson as any)[ext.pkg].baseClass === 'Native'
                          return (
                            <div className="flex flex-wrap gap-1 items-center">
                              {fullySupported ? (
                                <Badge
                                  variant="outline"
                                  className="text-[9px] px-1.5 py-0 h-4 bg-emerald-500/10 text-emerald-500 border-emerald-500/20 gap-1 font-bold"
                                >
                                  SUPPORTED
                                </Badge>
                              ) : (
                                <Badge
                                  variant="outline"
                                  className="text-[9px] px-1.5 py-0 h-4 bg-amber-500/10 text-amber-500 border-amber-500/20 gap-1 font-bold"
                                >
                                  NOT YET SUPPORTED
                                </Badge>
                              )}
                              {isHandwritten && (
                                <Badge
                                  variant="outline"
                                  className="text-[9px] px-1.5 py-0 h-4 bg-blue-500/10 text-blue-500 border-blue-500/20 font-medium whitespace-nowrap"
                                >
                                  Native ⚡
                                </Badge>
                              )}
                              <Badge
                                variant="outline"
                                className="text-[9px] px-1.5 py-0 h-4 bg-blue-500/10 text-blue-500 border-blue-500/20 uppercase font-medium truncate max-w-[60px]"
                              >
                                {native.theme ||
                                  (generatedSourcesJson as any)[ext.pkg]?.baseClass ||
                                  'CUSTOM-API'}
                              </Badge>
                            </div>
                          )
                        ...
                        <Button
                          ...
                          className={cn(
                            ...
                            installedPkgSet.has(ext.pkg)
                              ...
                              : 'text-primary hover:underline hover:bg-transparent'
                          )}
                        >
                          {installStatuses[ext.pkg] === 'loading' && (
                            <Loader2 className="animate-spin h-3 w-3 mr-1" />
                          )}
                          {installStatuses[ext.pkg] === 'success' && (
                            <Check className="h-3 w-3 mr-1 text-green-500" />
                          )}
                          {installStatuses[ext.pkg] === 'error' && (
        ...
        >
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground leading-relaxed">
              This will install{' '}
              <span className="font-bold text-foreground">{pendingInstallCount}</span> extensions.
              Are you sure you want to continue?
            </p>
            <div className="flex gap-3 justify-end mt-2">
              <Button variant="ghost" onClick={() => setIsConfirmInstallOpen(false)}>
                Cancel
              </Button>
              <Button
                variant="primary"
                ...
                onClick={() => {
                  ...
                  const toInstall = filteredExtensions.filter(
                    (ext) => !installedPkgSet.has(ext.pkg)
                  )
                  executeBulkInstall(toInstall)
                }}
              >
