...
export function MangaDetailsView({
  ...
  const handleDownload = async (chapter: Chapter) => {
    ...
    try {
      ...
      addToast({ type: 'success', title: 'Download Started', message: `Chapter ${chapter.number} is being downloaded.` })
    } catch (err: unknown) {
  ...
  const handleBulkDownload = (type: 'all' | 'unread' | 'next-5' | 'next-10') => {
    ...
    if (toDownload.length === 0) {
      addToast({ type: 'info', message: 'All selected chapters are already downloaded or in queue.' })
      return
    }
    ...
    addToast({ type: 'success', title: 'Bulk Download', message: `Added ${toDownload.length} chapters to the queue.` })
  }
        ...
        <ChapterEpisodeList 
          ...
          onRemoveDownload={async (chapter) => {
            ...
            try {
              ...
              addToast({ type: 'success', message: `Removed download for Chapter ${chapter.number}` })
            } catch (err) {
              const msg = err instanceof Error ? err.message : String(err)
              console.error(`[MangaDetailsView] Remove download failed:`, err)
              addToast({ type: 'error', title: 'Removal Failed', message: msg })
            }
          }}
          onRefresh={refetch}
        />
