  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        useExtensionStore.getState()._init(installed as any[], settingsMap as Record<string, string>)
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-[#09090b] text-white p-6 text-center selection:bg-primary/30">
        ...
        <p className="text-zinc-500 mb-8 max-w-sm font-medium">The application failed to synchronize with the local database. This might be due to a filesystem lock or corrupted cache.</p>
        <button 
