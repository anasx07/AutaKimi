...
export class MangaThemesiaSource implements ISourceAdapter {
  ...
  private parseMangaPage(html: string | null): MangaPage {
    ...
    const listSelector =
      this.customSelectors?.searchMangaSelector ||
    ...
    $(listSelector).each((_, el) => {
      ...
      const title = titleSelector
        ? $(el).find(titleSelector).text().trim()
  ...
  async fetchMangaDetails(manga: Manga): Promise<Manga> {
    ...
    const detailsSelector =
      this.customSelectors?.seriesDetailsSelector ||
    ...
    if (box.length) {
      ...
      const descSelector =
        this.customSelectors?.seriesDescriptionSelector ||
        '.desc, .entry-content[itemprop=description]'
      const coverSelector =
        this.customSelectors?.seriesThumbnailSelector ||
        '.infomanga > div[itemprop=image] img, .thumb img'
      const genreSelector =
        this.customSelectors?.seriesGenreSelector || 'div.gnr a, .mgen a, .seriestugenre a'
      ...
      const genres: string[] = []
      box.find(genreSelector).each((_, el) => {
        genres.push($(el).text())
      })
      ...
      return {
        ...
        genres,
        status:
          box.find('.tsinfo .imptdt:contains(status) i, .fmed b:contains(status)+span').text() ||
          manga.status
      }
    }
  ...
  async fetchChapters(mangaUrl: string): Promise<Chapter[]> {
    ...
    const chapterSelector =
      this.customSelectors?.chapterListSelector || 'div.bxcl li, div.cl li, #chapterlist li'
    ...
    items.each((_, el) => {
      ...
      const title =
        $(el).find('.chap-num').text().trim() ||
        $(el).find('.lch a, .chapternum').text().trim() ||
        (a.length > 0 ? a.text().trim() : $(el).text().trim())
...
function getImgAttr(el: cheerio.Cheerio<any>): string {
  return (
    el.attr('data-lazy-src') || el.attr('data-src') || el.attr('data-cfsrc') || el.attr('src') || ''
  )
}
