...
export class MadaraSource implements ISourceAdapter {
  ...
  async fetchChapters(mangaUrl: string): Promise<Chapter[]> {
    ...
    let chapterSelector = this.customSelectors?.chapterListSelector || '.wp-manga-chapter a, li.wp-manga-chapter a, .chapter-item .chapter a, .li .chapter a, .chapter-link'
    ...
    if (rows.length === 0) {
      ...
      if (mangaId) {
        ...
        if (ajaxHtml && ajaxHtml.length > 30) {
          console.log(`[MadaraSource] Ajax success (manga_get_chapters), length: ${ajaxHtml.length}. First chars: ${ajaxHtml.substring(0, 100).replace(/\n/g, '')}...`)
        ...
        if (rows.length === 0) {
           console.log(`[MadaraSource] Standard AJAX failed (Rows: ${rows.length}, HTML Length: ${ajaxHtml?.length}), trying alternative actions for ID: ${mangaId}...`)
           const fallbacks = [
           ...
           for (const body of fallbacks) {
              ...
              if (res && res.length > 50) {
                 console.log(`[MadaraSource] Fallback AJAX success (${body.split('&')[0]}), length: ${res.length}. Parsing...`)
                 $ = cheerio.load(res)
                 rows = $(chapterSelector)
                 if (rows.length > 0) {
                    console.log(`[MadaraSource] Success on fallback (${body.split('&')[0]}) with ${rows.length} chapters!`)
                    break;
                 }
              } else {
                 console.log(`[MadaraSource] Fallback AJAX failed or short (${body.split('&')[0]}), res: ${res?.substring(0, 50).replace(/\n/g, '')}...`)
              }
           }
        }
        ...
        if (rows.length === 0) {
         ...
         console.warn(`[MadaraSource] Could not find manga ID in HTML for ${mangaUrl}. Holders checked: input.rating-post-id, #manga-chapters-holder, etc.`)
      }
    }
