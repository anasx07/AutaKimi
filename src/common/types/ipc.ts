...
export interface ElectronApi {
  ...
  db: {
    ...
    addExtension: (args: { pkg: string; code?: string; name?: string; baseUrl?: string; lang?: string; icon?: string }) => Promise<IpcResult<{ success?: boolean; error?: string }>>;
    getExtension: (pkg: string) => Promise<IpcResult<any | null>>;
    removeExtension: (pkg: string) => Promise<IpcResult<boolean>>;
    getLibrary: (args?: { limit?: number; offset?: number; type?: string }) => Promise<IpcResult<any[]>>;
    ...
    setSetting: (key: string, value: string) => Promise<IpcResult<{ success?: boolean; error?: string }>>;
    getProgress: (mangaId: string) => Promise<IpcResult<{ chapterId: string, isRead: boolean, lastPage: number }[]>>;
    updateProgress: (args: { mangaId: string; chapterId: string; isRead: boolean; lastPage?: number }) => Promise<IpcResult<{ success?: boolean; error?: string }>>;
    addHistory: (args: { mangaId: string; mangaTitle?: string; mangaCover?: string; mangaUrl?: string; chapterId: string; chapterTitle?: string; startedAt: string; durationSeconds?: number; pkg?: string; type?: 'manga' | 'anime' }) => Promise<IpcResult<{ success?: boolean; error?: string }>>;
    getHistory: (args?: { limit?: number; offset?: number; type?: string } | number) => Promise<IpcResult<any[]>>;
    deleteHistoryEntry: (id: number) => Promise<IpcResult<{ success?: boolean; error?: string }>>;
    deleteHistoryByManga: (mangaId: string) => Promise<IpcResult<{ success?: boolean; error?: string }>>;
    clearHistory: () => Promise<IpcResult<{ success?: boolean; error?: string }>>;
    getChapters: (mangaId: string) => Promise<IpcResult<any[]>>;
    saveChapters: (args: { mangaId: string; chapters: any[] }) => Promise<IpcResult<{ success?: boolean; error?: string }>>;
  ...
  executeExtension: (args: { pkg: string; code: string; contextArgs?: any }) => Promise<IpcResult<any>>;
  installExtension: (ext: any, repoUrl: string) => Promise<IpcResult<{ success?: boolean; error?: string }>>;
  openExternal: (url: string) => Promise<void>;
  download: {
    start: (args: { mangaId: string; chapterId: string; pageUrls: string[] }) => Promise<IpcResult<boolean>>;
