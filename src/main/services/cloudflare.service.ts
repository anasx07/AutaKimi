...
export class CloudflareService {
  ...
  private bypasses = new Map<string, Promise<string | null>>()
  ...
  private setupSharedListeners(): void {
    ...
    this.cfSession.webRequest.onBeforeSendHeaders({ urls: ['*://*/*'] }, (details, callback) => {
      const wcId = details.webContentsId
      // Get UA from map if it's a window, otherwise use the last known cleaned UA (stored at -1)
      const ua =
        (wcId !== undefined ? this.activeWindows.get(wcId) : null) ||
        this.activeWindows.get(-1) ||
        session.defaultSession.getUserAgent()

      details.requestHeaders['Sec-CH-UA'] =
        `"Google Chrome";v="${chromeVer}", "Chromium";v="${chromeVer}", "Not_A Brand";v="8"`
    ...
    this.cfSession.webRequest.onHeadersReceived({ urls: ['*://*/*'] }, (_details, callback) => {
  ...
  private isCfChallengePage(html: string): boolean {
    ...
    const isCf =
      ...
      (lower.includes('cloudflare') && (lower.includes('verifying') || lower.includes('checking')))
  ...
  async fetchHtmlViaBrowser(targetUrl: string): Promise<string | null> {
    ...
    // 1. Strict Domain Lock: if a bypass is already running, wait for it
    //    then try an optimistic fetch with the newly obtained cookies.
    const existingBypass = this.bypasses.get(domain)
    if (existingBypass) {
      console.log(`[CloudflareService] Waiting for existing bypass for ${domain}...`)
      try {
        await existingBypass
      } catch {
        /* bypass failed, we'll try our own optimistic fetch anyway */
      }
      // After the bypass finishes, try optimistic fetch with new cookies
      try {
        const ua = this.activeWindows.get(-1) || session.defaultSession.getUserAgent()
        ...
        if (html && !this.isCfChallengePage(html)) {
          console.log(`[CloudflareService] ✓ Post-wait fetch successful for ${targetUrl}`)
          return html
        }
      } catch {
        /* fall through to attempt our own bypass */
      }
    }

    // 2. Optimistic check: try a direct fetch if we have a valid cookie
    const existingCookie = await this.getClearanceCookie(domain)
    if (existingCookie && (existingCookie.expirationDate || 0) > Date.now() / 1000) {
      console.log(
        `[CloudflareService] Found valid cf_clearance for ${domain}, trying optimistic fetch...`
      )
      try {
        const ua = this.activeWindows.get(-1) || session.defaultSession.getUserAgent()
        const response = await this.cfSession.fetch(targetUrl, {
    ...
    // 3. Acquire the domain lock SYNCHRONOUSLY before any async browser work.
    //    Use a deferred pattern so the promise is in the map before we await anything.
    let resolveBypass!: (val: string | null) => void
    let rejectBypass!: (err: unknown) => void
    const bypassPromise = new Promise<string | null>((resolve, reject) => {
      resolveBypass = resolve
      rejectBypass = reject
    })
    this.bypasses.set(domain, bypassPromise)
      ...
      win.webContents.on('dom-ready', async () => {
        ...
        try {
          ...
          if (this.isCfChallengePage(html)) {
            console.log(
              `[CloudflareService] Still on CF challenge page... (Length: ${html.length})`
            )
            return
          }

          console.log(
            `[CloudflareService] ✓ Extracted ${html.length} chars from ${lastNavigatedUrl || url}`
          )
          done(html)
        } catch {
          /* wait */
