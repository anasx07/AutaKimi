...
async function main() {
  try {
    ...
    try {
      ...
      console.log(`${C.yellow}${C.bright}Please Fix these errors and Come again for Release${C.reset}\n`);
      return;
    }
    ...
    while (true) {
      try {
        ...
        console.log(`${C.green}  (1)${C.reset} ${C.bright}Full Release${C.reset}       ${C.dim}(Stage All -> Commit -> Tag -> Push)${C.reset}`);
        console.log(`${C.yellow}  (2)${C.reset} ${C.bright}Version Only${C.reset}       ${C.dim}(Stage package.json -> Tag -> Push)${C.reset}`);
        console.log(`${C.magenta}  (3)${C.reset} ${C.bright}Tag Only${C.reset}           ${C.dim}(Skip package.json, push vTag only)${C.reset}`);
        ...
        if (choice === '1' || choice === '2') {
          newVersion = await question(`${C.cyan}${C.bright}» Target Version (e.g. 1.0.6):${C.reset} `);
          if (!newVersion) {
          ...
          const tagSuffix = await question(`${C.cyan}${C.bright}» Tag Name (current is v${pkg.version}):${C.reset} v`);
          newVersion = tagSuffix || pkg.version;
        }

        // 4. Confirmation
        console.log(`\n${C.bg} CONFIRMATION ${C.reset}`);
        console.log(`${C.dim}Mode: ${choice === '1' ? 'Full' : choice === '2' ? 'Version' : 'Tag-Only'}${C.reset}`);
        console.log(`${C.dim}Task: pushing ${C.reset}${C.green}${C.bright}v${newVersion}${C.reset}${C.dim} to GitHub...${C.reset}`);
        ...
        if (choice === '1') {
          ...
          execSync('git add .');
          const msg = await question(`${C.cyan}${C.bright}» Commit Message (default: v${newVersion}):${C.reset} `);
