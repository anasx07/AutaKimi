...
async function main() {
  ...
  const answer1 = await ask(
    rl,
    `  ${c.yellow}?${c.reset} Proceed with clearing? ${c.dim}[Y/n]${c.reset} `
  )
  ...
  while (!ready) {
    const answer2 = await ask(
      rl,
      `  ${c.yellow}?${c.reset} Press ${c.bold}Enter${c.reset} once you've placed the icon (or type ${c.bold}q${c.reset} to quit): `
    )
  ...
  console.log()
  console.log(
    `  ${c.dim}Next step: run${c.reset} ${c.bold}npm run build:win${c.reset} ${c.dim}to package the app.${c.reset}`
  )
  console.log()
